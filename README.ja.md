...
3.  **高精度・高速なランダムアセット- **v2.3.2**: **引用符付きパスのサポート** (`strip('"')`)、プロンプト読み込みの **デバッグログ出力** を実装。
...
言語設定・正規表現のキャッシュ化により、動作の安定性とパフォーマンスを向上させました。
...
8.  **統合型 プロンプト&LoRAマネージャー (v2.2.0)**: キャラクタ、シチュエーション、および3つのワイルドカードスロット（計5つ）を管理。**前後どちらに追加するか**も選択可能です。
