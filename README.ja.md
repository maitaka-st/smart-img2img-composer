...
3.  **LoRAウェイトの一括調整 (v2.4.1)**: プリセット内のすべてのLoRAウェイトを一律で微調整できるグローバルスライダーを搭載。
4.  **生成結果の自動フォルダ振り分け (v2.4.1)**: プリセット名、セクション名、日付などで出力先を自動サブフォルダ化。
5.  **精緻なヘルスチェック (v2.4.2)**: 設定されたパスが有効かどうかを、各入力欄の横に **✅/❌ アイコン** でリアルタイムに表示。
6.  **完全多言語対応 (i18n)**: UIのすべての要素が英語と日本語に完全対応。
7.  **高精度な自動プロンプト生成**: WD14 Tagger連携による自動タグ抽出機能を搭載。
